...
export const predictionMarkets = ({
    ...
    getTools: async (
        ...
        console.log("Market Address:", process.env.MARKET_ADDRESS);
        console.log("Mode Token Address:", process.env.MODE_TOKEN_ADDRESS);
            ...
            },
            {
                ...
                execute: async (params: MarketParams) => {
                    try {
                        ...
                        // Check if wallet is a market creator
                        const canCreate = await isMarketCreator(address);
                        if (!canCreate) {
                            throw new Error(
                        ...
                        const marketCreatedEvent = await publicClient.getLogs({
                            address: factoryAddress,
                            event: {
                                type: "event",
                                name: "MarketCreated",
                        ...
                        if (!marketCreatedEvent.length) {
                            throw new Error("Market creation event not found");
                        }

                        const event = marketCreatedEvent[0];
                        const { args } = decodeEventLog({
                            abi: [
                                parseAbiItem(
                                    "event MarketCreated(bytes32 marketId, address indexed marketAddress, string question, uint256 endTime, address collateralToken, uint256 virtualLiquidity)"
